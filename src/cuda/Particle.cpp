...
void Particle::updateParticleVelocity(Vector3d forces, double h) {
    Vector3d newVelocity = getVelocity() + ( h * (1.0/m) * forces);
    v = newVelocity;
}

void Particle::updateParticlePosition(Vector3d forces, double h){
    Vector3d newPos = getPosition() + (h * v);
    x = newPos;
